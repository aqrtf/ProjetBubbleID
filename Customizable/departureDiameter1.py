def ComputeDepartureDiameter(self,
    ...
    dwell_csv = os.path.join(self.savefolder, f"dwell6_{self.extension}.csv")       # Attachement et depart des bulles
    ...
    if out_csv is None:
        out_csv = os.path.join(self.savefolder, f"departure2_{self.extension}.csv")
    ...
    if not os.path.isfile(dwell_csv):
        raise FileNotFoundError(f"{dwell_csv} non trovato.")
    ...
    # Chargement du csv dwell
    df_dwell = pd.read_csv(dwell_csv)
    # On extrait le premier tid de la chaine de caractere
    # Exemple de DataFrame
    df_dwell["first_num"] = df_dwell["bubble_id"].str.extract(r'^(\d+)').astype(int)
    ...
    def _find_departure(tid, df_dwell):
        """
        TROUVE LE MOMENT DE DÉTACHEMENT D'UNE BULLE
        On va le lire dans dwell_ .csv
        Retourne: (attach_start, last_attached, detach_frame)
        """
        row = df_dwell[df_dwell["first_num"] == tid]
        if len(row)>1:
            raise(f"Error: multiple rows for tid {tid}")
        elif len(row) == 0:
            attach_start = None
            last_attached = None
            detach_frame = None
        else:
            attach_start = row.iloc[0].at["attach_start_frame"]
            last_attached = row.iloc[0].at["last_attach_frame"]
            detach_frame = row.iloc[0].at["detach_frame"]
                
        return attach_start, last_attached, detach_frame
    ...
    for tid in range(n_tracks):
        ...
        attach_start, last_attached, detach_frame = _find_departure(tid, df_dwell)
        ...
        base = {
            "bubble_id": tid,
            "attach_start_frame": int(attach_start) if not pd.isna(attach_start) else None,
            "last_attached_frame": int(last_attached) if not pd.isna(last_attached) else None,
            "detach_frame": int(detach_frame) if not pd.isna(detach_frame) else None,
            "note": "ok" if not pd.isna(detach_frame) else "no_detach_found",
            "k": k, 
            "fit_kind": fit_kind
        }
